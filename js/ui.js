...
export function renderDoaListUI(doaList) {
    ...
    if (!Array.isArray(doaList) || doaList.length === 0) {
        container.innerHTML = `<div class=\"p-6 text-center text-xs text-slate-400\">Doa tidak ditemukan.</div>`;
        return;
    }

    container.innerHTML = doaList.map((doa, index) => {
        const title = doa.judul || doa.nama || doa.title || doa.doa || `Doa ${index + 1}`;
        const arab = doa.arab || doa.ar || doa.arabic || '';
        const latin = doa.latin || doa.idn || doa.transliterasi || '';
        const arti = doa.arti || doa.terjemah || doa.translation || doa.id || '';
        const kat = doa.kat || doa.grup || doa.kategori || 'Harian';
        return `
        <div class=\"p-4 bg-white rounded-2xl shadow-sm border border-slate-100 space-y-3\">
            <div class=\"flex items-start justify-between gap-2\">
                ...
                <span class=\"text-[10px] font-medium px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-100\">${kat}</span>
            </div>
            ${arab ? `<p class=\"text-right font-arabic text-xl leading-loose text-slate-800 pt-1\">${arab}</p>` : ''}
            ${latin ? `<p class=\"text-xs text-emerald-700 font-medium italic\">${latin}</p>` : ''}
            ${arti ? `<p class=\"text-xs text-slate-600 leading-relaxed\">${arti}</p>` : ''}
        </div>`;
    }).join('');
}
...
export function renderSurahListUI(list, openModalFn) {
    const container = document.getElementById('surah-list') || document.getElementById('surah-list-container');
    if (!container) return;
